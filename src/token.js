...
const { FANYI_BAIDU_URL } = require("./config")
...
async function update() {
  ...
  if (params && params.expires > new Date().getTime()) {
    ...
    try {
      const cookieResp = await rp({
        uri: FANYI_BAIDU_URL,
        headers: { 'User-Agent': new userAgents({ deviceCategory: 'desktop' }).toString() },
        resolveWithFullResponse: true
      })
      let cookie = cookieResp.headers['set-cookie']
      if (cookie && cookie.length > 0) {
        for (const str of cookie) {
      ...
      const gtk = tokenResp.match(regExp.gtk)
      let token = tokenResp.match(regExp.token)
...
async function get(text) {
  try {
    const { token, cookie } = await update()
    return { sign: e(text), token, cookie }
  } catch (error) {
    throw error
  }
}
